...
def _gdpr_action(method, scope, user, query_uuid=None):
    ...
    with requests_mock.Mocker() as req_mock:
        gdpr_profile_url = reverse("api-1.0.0:get_user_info", kwargs={"user_id": query_uuid})
        auth_header = get_api_token_for_user_with_scopes(
            user.uuid, [scope], req_mock
        )
...
def _gdpr_delete(api_client, user, query_uuid=None):
    return _gdpr_action(api_client.delete, settings.GDPR_API_DELETE_SCOPE, user, query_uuid)
...
def test_gdpr_delete_with_query_scope(client, user):
    assert _gdpr_action(client.delete, settings.GDPR_API_QUERY_SCOPE, user).status_code == 403


@pytest.mark.django_db
def test_gdpr_query_with_delete_scope(client, user):
    assert _gdpr_action(client.get, settings.GDPR_API_DELETE_SCOPE, user).status_code == 403
