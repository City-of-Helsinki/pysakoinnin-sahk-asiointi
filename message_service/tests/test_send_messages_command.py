...
@pytest.mark.django_db
def test_send_messages_command_send_failed(message):
    message.queued = True
    message.save()

    with patch.object(Message, "send", return_value=False) as mock_send:
        out = StringIO()
        call_command("send_messages", stdout=out)

    mock_send.assert_called_once()
    output = out.getvalue()
    assert f"Message (pk={message.pk}) failed" in output
