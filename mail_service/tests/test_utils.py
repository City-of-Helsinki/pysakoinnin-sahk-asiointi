...
from mail_service.utils import (
    events,
    extend_due_date_mail_constructor,
    headers,
    mail_constructor,
)
...
class TestMailUtils(TestCase):
    def test_extend_due_date_mail_constructor(self):
        ...
        mail = extend_due_date_mail_constructor(
            lang="FI", new_due_date=new_due_date, mail_to=mail_to
        )
        ...
        mail = extend_due_date_mail_constructor(
            lang="EN", new_due_date=new_due_date, mail_to=mail_to
        )
        assert mail.subject == headers["EN"]

        mail = extend_due_date_mail_constructor(
            lang="SV", new_due_date=new_due_date, mail_to=mail_to
        )
        assert mail.subject == headers["SV"]

        # incorrect lang, default to FI
        mail = extend_due_date_mail_constructor(
            lang="INCORRECT_LANG", new_due_date=new_due_date, mail_to=mail_to
        )
        assert mail.subject == headers["FI"]
    ...
    def test_mail_constructor(self):
        ...
        mail = mail_constructor(
            event="received", lang="INCORRECT_LANG", mail_to=mail_to
        )
        assert mail.subject == headers["FI"]
